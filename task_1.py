...
class Sweets_jar:
    ...
    def take_candy(self, take_ch: int, take_caramels: int, min_caramels: int = 2) -> str:
        ...
        if take_caramels + self.caramels  < min_caramels:
            raise ValueError("Нельзя оставить в банке меньше минимального допустимого остатка")
        if take_ch > self.ch_cand or take_caramels > self.caramels:
            raise ValueError("Нельзя взять больше конфет, чем находится в банке")
    ...
    def put_candy(self, put_ch: int, put_caramels: int) -> str:
        ...
        if not isinstance(put_ch, int) or not isinstance(put_caramels, int):
            raise TypeError("Количество добавленных конфет должно иметь тип int")
        ...
        if self.ch_cand + self.caramels + put_ch + put_caramels > self.max_cand:
            raise ValueError("Нельзя положить в банку больше конфет, чем она вмещает")
...
class Coffee_cup():
    def __init__(self, sort: str, base_price: int = 90):
        ...
        if sort != "arabic"  and sort != "robusta":
            raise ValueError("Нужно выбрать один из двух сортов кофе: arabic или robusta")     
    ...
    def add_milk(self, value: bool, price_premium: int = 50) -> int:
        ...
        """
        if type(value) != bool:
            raise TypeError("Значение наличия или отсутствия молока должно иметь тип bool")
    ...
    def add_syrop(self, value: bool, price_premium: int = 20) -> int:
        ...
        """
        if type(value) != bool:
            raise TypeError("Значение наличия или отсутствия сиропа должно иметь тип bool")
